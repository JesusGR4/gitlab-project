...
def test_create_product(product, redis_client, service_container):
    ...
    assert product['flow_capacity'] == int(stored_product[b'flow_capacity'])
    assert product['width'] == (
        int(stored_product[b'width']))
...
@pytest.mark.parametrize('product_overrides, expected_errors', [
    ...
    (
        {'flow_capacity': 'not-an-integer'},
        {'flow_capacity': ['Not a valid integer.']}
    ),
    (
...
@pytest.mark.parametrize('field', [
    'id', 'title', 'width', 'flow_capacity', 'in_stock'])
