...
make-test-gateway:
	sed -i "s/desatranques\/orders/${DOCKER_REGISTRY}\/${CI_PROJECT_NAME}\/orders:${TEMPORAL_VERSION}/g" docker-compose-ci-test-gateway.yml
	sed -i "s/desatranques\/products/${DOCKER_REGISTRY}\/${CI_PROJECT_NAME}\/products:${TEMPORAL_VERSION}/g" docker-compose-ci-test-gateway.yml
	sed -i "s/desatranques\/gateway/${DOCKER_REGISTRY}\/${CI_PROJECT_NAME}\/gateway:${TEMPORAL_VERSION}/g" docker-compose-ci-test-gateway.yml
	sed -i "s/desatranques\/test/${DOCKER_REGISTRY}\/${CI_PROJECT_NAME}\/test:${TEMPORAL_VERSION}/g" test/docker-compose-ci-test-gateway.yml
